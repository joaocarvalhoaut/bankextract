...
export default function LoginScreen({ onSignIn, onSignUp, loading, error, onBackToLanding }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (mode === 'signin') {
        ...
        if (!onSignUp) throw new Error('Fluxo de criacao de conta indisponivel no momento.');
  ...
  return (
    ...
    >
      ...
      <div aria-hidden="true" className="pointer-events-none absolute" style={{ top: '-10%', left: '50%', transform: 'translateX(-50%)', width: '700px', height: '400px', background: 'radial-gradient(ellipse at center, rgba(0,93,255,0.22) 0%, transparent 70%)', borderRadius: '50%' }} />

      <div className="relative z-10 w-full max-w-md">
        {onBackToLanding ? (
          <button type="button" onClick={onBackToLanding} className="mb-6 inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold text-slate-300 hover:text-white transition-colors" style={{ border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(255,255,255,0.05)' }}>
            <ArrowLeft size={12} />
            Voltar para landing
          </button>
        ) : null}

        <div className="rounded-3xl p-8" style={{ background: 'rgba(13,27,46,0.85)', border: '1px solid rgba(0,93,255,0.25)', backdropFilter: 'blur(20px)', boxShadow: '0 24px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(20,216,255,0.08) inset' }}>
