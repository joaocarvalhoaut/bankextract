          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            {results ? (
              mocked ? (
                <p className="text-sm text-amber-700">
                  <span className="font-semibold">{sentCount} cobranca(s)</span> preparada(s) em modo teste.
                </p>
              ) : (
                <p className="text-sm text-slate-600">
                  <span className="font-semibold text-emerald-700">{sentCount} enviado(s)</span>
                  {failCount > 0 ? <span className="ml-3 font-semibold text-red-600">{failCount} com erro</span> : null}
                </p>
              )
            ) : (
              <p className="text-xs text-slate-500">Revise o texto e confirme somente quando a mensagem estiver pronta.</p>
            )}
            ...
            <div className="flex gap-2">
              <button
                ...
                onClick={onClose}
                disabled={sending}
                className="rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:bg-slate-50 disabled:opacity-50"
              >
              ...
              {!results ? (
                ...
                >
                  {sending ? (
                    ...
                    <>
                      <Send size={14} />
                      Enviar {records.length} cobranca(s)
                    </>
                  )}
                </button>
              ) : null}
