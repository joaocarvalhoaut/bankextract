      ...
      <div className="modal-shell w-full max-w-md overflow-hidden rounded-[28px]">
        <div className="flex items-center justify-between border-b border-slate-700 px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-semibold text-slate-50">{modalData.modo === 'novo' ? 'Novo representante' : 'Editar representante'}</h3>
        ...
        <div className="space-y-4 p-5">
          <label className="block text-xs font-medium text-slate-200">
            ...
            <input value={modalData.rep.nome} onChange={(event) => onChange('nome', event.target.value)}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <label className="block text-xs font-medium text-slate-200">
              ...
              <input value={modalData.rep.telefone} onChange={(event) => onChange('telefone', event.target.value)}
                className="w-full rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-sm text-slate-50 placeholder-slate-500 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/30" />
            </label>
            <label className="block text-xs font-medium text-slate-200">
              <span className="mb-1.5 flex items-center gap-1.5"><Mail size={12} /> E-mail</span>
              <input value={modalData.rep.email} onChange={(event) => onChange('email', event.target.value)}
                className="w-full rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-sm text-slate-50 placeholder-slate-500 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/30" />
            </label>
          </div>

          <label className="block text-xs font-medium text-slate-200">
            <span className="mb-1.5 flex items-center gap-1.5"><MessageSquare size={12} /> Observacoes</span>
            <textarea value={modalData.rep.observacao} onChange={(event) => onChange('observacao', event.target.value)} rows={3}
        ...
        <div className="flex items-center justify-between border-t border-slate-700 bg-slate-800/40 px-5 py-4">
          {modalData.modo === 'editar' ? (
            <button onClick={() => onDelete(modalData.rep.id)} className="inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium text-red-400 hover:bg-red-500/10">
