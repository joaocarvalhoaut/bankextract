...
function AuditRow({ log, onReprocess }) {
  ...
  return (
    <div className={`border rounded-lg mb-2 overflow-hidden ${STATUS_META[status]?.border || 'border-gray-200'}`}>
      <button
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {log.ocr_used && <span className="text-xs text-blue-600 font-medium">OCR</span>}
            <span className="text-xs text-gray-500 font-mono truncate">{log.charge_id || log.registro_id || '—'}</span>
          </div>
          ...
          <dl className="grid grid-cols-2 gap-x-4 gap-y-1.5 mt-3 text-xs">
            {log.blocked_reason && (
              <div className="col-span-2">
                <dt className="text-gray-500">Motivo do bloqueio</dt>
                <dd className="font-medium text-red-600">{log.blocked_reason}</dd>
              </div>
            )}
            {log.boleto_file_id && (
              <div>
                <dt className="text-gray-500">Arquivo boleto</dt>
                <dd className="font-mono truncate">{log.boleto_file_id}</dd>
              </div>
            )}
            {log.boleto_strategy && (
              <div>
                <dt className="text-gray-500">Estratégia</dt>
                <dd>{log.boleto_strategy}</dd>
              </div>
            )}
            {log.boleto_second_score != null && (
              <div>
                <dt className="text-gray-500">2º score</dt>
                <dd><ScoreBadge score={log.boleto_second_score} /></dd>
              </div>
            )}
            {log.ocr_source && (
              <div>
                <dt className="text-gray-500">Fonte OCR</dt>
                <dd>{log.ocr_source}</dd>
              </div>
            )}
            {log.pdf_validation_reason && (
              <div>
                <dt className="text-gray-500">Validação PDF</dt>
            ...
            {log.pdf_hash && (
              <div className="col-span-2">
                <dt className="text-gray-500">Hash PDF (SHA-256)</dt>
  ...
  return (
    <div className="flex flex-wrap gap-2 mb-4">
      {items.map(([status, count]) => (
        <div
          key={status}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm font-medium ${STATUS_META[status]?.bg || 'bg-gray-50'} ${STATUS_META[status]?.border || 'border-gray-200'} ${STATUS_META[status]?.color || 'text-gray-600'}`}
        >
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-gray-800">Status de correspondência — Boletos</h2>
        </div>
        <button
          onClick={load}
          disabled={loading}
          className="p-1.5 rounded hover:bg-gray-100 text-gray-500 transition-colors disabled:opacity-40"
          title="Atualizar"
        >
      ...
      <div className="px-5 py-4">
        ...
        {!loading && !error && filtered.length > 0 && (
          <div>
            ...
            {logs.length === limit && (
              <p className="text-xs text-center text-gray-400 mt-2">Exibindo os {limit} registros mais recentes.</p>
            )}
          </div>
        )}
      </div>
